...
AUTH0_DOMAIN = os.getenv('AUTH0_DOMAIN', 'mhnd.eu.auth0.com')
...
get_token_auth_header() method
    it get the header from the request
        it raise an AuthError if no header is present
    it split bearer and the token
...
verify_decode_jwt(token) method
    ...
    it's an Auth0 token with key id (kid)
